...
imagen.scaleToFit(width, height);
        document.add(imagen);
            ...
            PdfPCell valorCell = new PdfPCell(new Phrase("Valor (COP) =  "+"$"+formato.format(valor)));
    ...
    table.addCell(new PdfPCell(new Phrase("Peso (KG)", HEADER_FONT)) {{
        setHorizontalAlignment(Element.ALIGN_CENTER);
    }});
    table.addCell(new PdfPCell(new Phrase("Número de Cuenta", HEADER_FONT)) {{
        setHorizontalAlignment(Element.ALIGN_CENTER);
    }});
    table.addCell(new PdfPCell(new Phrase("Material(Cantidad)", HEADER_FONT)) {{
        setHorizontalAlignment(Element.ALIGN_CENTER);
    }});
    table.addCell(new PdfPCell(new Phrase("Valor (COP)", HEADER_FONT)) {{
        setHorizontalAlignment(Element.ALIGN_CENTER);
    }});
...
//idInventario, peso, Descripcion, idMaterial, Valor, fecha
    ...
    private static void addTableData(PdfPTable table, List<Inventario> inventarioList) {
       ...
       for (Inventario inventario : inventarioList) {
        ...
        table.addCell(new PdfPCell(new Phrase(String.valueOf("$"+formato.format(inventario.getValor())), DATA_FONT)) {{
            setHorizontalAlignment(Element.ALIGN_CENTER);
        }});
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
        Timestamp fecha = inventario.getFecha();
