...
public class CajaMenorController {
    ...
    public List<MovimientoCajaMenor> obtenerMovimientosCajaMenor() throws SQLException {
        ...
        try (Connection connection = conexion.estableceConexion(); PreparedStatement statement = connection.prepareStatement(query); ResultSet resultSet = statement.executeQuery()) {
            while (resultSet.next()) {
                ...
                MovimientoCajaMenor movimiento = new MovimientoCajaMenor(id, fecha, tipoMovimientoId, monto, descripcion,esgasto);
    ...
    public Double obtenerSumaMontoMovimientosCajaMenor() throws SQLException {
        double sumaMonto = 0.0;
        String query = "SELECT SUM(monto) AS suma_monto FROM caja_menor";
        try (Connection connection = conexion.estableceConexion(); PreparedStatement statement = connection.prepareStatement(query); ResultSet resultSet = statement.executeQuery()) {
            if (resultSet.next()) {
                sumaMonto = resultSet.getDouble("suma_monto");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
public LocalDate obtenerFechaPrimerRegistro() throws SQLException {
    LocalDate fechaPrimerRegistro = null;
    String query = "SELECT fecha FROM caja_menor ORDER BY fecha ASC LIMIT 1";
...
public LocalDate obtenerFechaUltimoRegistro() throws SQLException {
    LocalDate fechaUltimoRegistro = null;
    String query = "SELECT fecha FROM caja_menor ORDER BY fecha DESC LIMIT 1";
    try (Connection connection = conexion.estableceConexion(); 
         PreparedStatement statement = connection.prepareStatement(query);
         ResultSet resultSet = statement.executeQuery()) {
        if (resultSet.next()) {
